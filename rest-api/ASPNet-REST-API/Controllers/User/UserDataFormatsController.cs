...
namespace ASPNet_REST_API.Controllers.Admin
{
    [ApiController]
    [Route("/User")]
    public class UserDataFormatsController : Controller
    {
        ...
        public IActionResult Index(string airtableWhere = null, string view = null, int maxPages = 5)
        {
            ...
            try
            {
                ...
                results = atdUser.GetDataFormats(payload)?.ToList();
            }
            catch (Exception ex)
            {
        ...
        public IActionResult Add(string airtableWhere = null, string view = null, int maxPages = 5)
        {
            ...
            try
            {
                ...
                using (var reader = new StreamReader(this.Request.Body))
                {
                    ...
                    if (bodyAsPayload != null) payload.DataFormat = bodyAsPayload.DataFormat.UserCleanForAdd();
                    if (payload.DataFormat is null) payload.DataFormat = JsonConvert.DeserializeObject<DataFormat>(body).UserCleanForAdd();
                    result = atdUser.AddDataFormat(payload).UserCleanForGet();
                }
            }
            catch (Exception ex)
            {
        ...
        public IActionResult Update(string airtableWhere = null, string view = null, int maxPages = 5)
        {
            ...
            try
            {
                using (var reader = new StreamReader(this.Request.Body))
                {
                    ...
                    if (bodyAsPayload != null) payload.DataFormat = bodyAsPayload.DataFormat.UserCleanForAdd();
                    if (payload.DataFormat is null) payload.DataFormat = JsonConvert.DeserializeObject<DataFormat>(body).UserCleanForAdd();
                    result = atdUser.UpdateDataFormat(payload)?.FirstOrDefault();
                }
        ...
        public IActionResult Delete(string id)
        {
            try {
                ...
                atdUser.DeleteDataFormat(payload);
