...
namespace ASPNet_REST_API.Controllers.Admin
{
    [ApiController]
    [Route("/User")]
    public class UserMOFNodesController : Controller
    {
        ...
        public IActionResult Index(string airtableWhere = null, string view = null, int maxPages = 5)
        {
            ...
            try
            {
                ...
                results = atdUser.GetMOFNodes(payload)?.ToList();
            }
            catch (Exception ex)
            {
        ...
        public IActionResult Add(string airtableWhere = null, string view = null, int maxPages = 5)
        {
            ...
            try
            {
                ...
                using (var reader = new StreamReader(this.Request.Body))
                {
                    ...
                    if (bodyAsPayload != null) payload.MOFNode = bodyAsPayload.MOFNode.UserCleanForAdd();
                    if (payload.MOFNode is null) payload.MOFNode = JsonConvert.DeserializeObject<MOFNode>(body).UserCleanForAdd();
                    result = atdUser.AddMOFNode(payload).UserCleanForGet();
                }
            }
            catch (Exception ex)
            {
        ...
        public IActionResult Update(string airtableWhere = null, string view = null, int maxPages = 5)
        {
            ...
            try
            {
                using (var reader = new StreamReader(this.Request.Body))
                {
                    ...
                    if (bodyAsPayload != null) payload.MOFNode = bodyAsPayload.MOFNode.UserCleanForAdd();
                    if (payload.MOFNode is null) payload.MOFNode = JsonConvert.DeserializeObject<MOFNode>(body).UserCleanForAdd();
                    result = atdUser.UpdateMOFNode(payload)?.FirstOrDefault();
                }
