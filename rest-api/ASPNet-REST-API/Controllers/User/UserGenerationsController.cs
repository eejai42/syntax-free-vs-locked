...
namespace ASPNet_REST_API.Controllers.Admin
{
    [ApiController]
    [Route("/User")]
    public class UserGenerationsController : Controller
    {
        ...
        public IActionResult Index(string airtableWhere = null, string view = null, int maxPages = 5)
        {
            ...
            try
            {
                ...
                results = atdUser.GetGenerations(payload)?.ToList();
            }
            catch (Exception ex)
            {
        ...
        public IActionResult Add(string airtableWhere = null, string view = null, int maxPages = 5)
        {
            ...
            try
            {
                ...
                using (var reader = new StreamReader(this.Request.Body))
                {
                    ...
                    if (bodyAsPayload != null) payload.Generation = bodyAsPayload.Generation.UserCleanForAdd();
                    if (payload.Generation is null) payload.Generation = JsonConvert.DeserializeObject<Generation>(body).UserCleanForAdd();
                    result = atdUser.AddGeneration(payload).UserCleanForGet();
                }
            }
            catch (Exception ex)
            {
        ...
        public IActionResult Update(string airtableWhere = null, string view = null, int maxPages = 5)
        {
            ...
            try
            {
                using (var reader = new StreamReader(this.Request.Body))
                {
                    ...
                    if (bodyAsPayload != null) payload.Generation = bodyAsPayload.Generation.UserCleanForAdd();
                    if (payload.Generation is null) payload.Generation = JsonConvert.DeserializeObject<Generation>(body).UserCleanForAdd();
                    result = atdUser.UpdateGeneration(payload)?.FirstOrDefault();
                }
        ...
        public IActionResult Delete(string id)
        {
            try {
                ...
                atdUser.DeleteGeneration(payload);
