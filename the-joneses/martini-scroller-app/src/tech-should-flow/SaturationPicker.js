...
const SaturationPicker = ({ transpilerNode, color, onChange, showPictures}) => {
    ...
    useEffect(() => {
        onChange(color);  // Ensure external changes are also pushed up
    }, [color, onChange]);
    ...
    return (
        <div style={{ position: "absolute", width: "10em", height: "11em", margin: "0.5em", border: 'solid 3px black', 
                      top: '3em', left: '10em',  backgroundColor: color, borderRadius: '0.45em', cursor: 'pointer' }} 
        className="outerHoverDiv"
        onClick={() => setShowPicker(true)}  // Set to true on click
        onMouseLeave={() => setShowPicker(false)}  
        >
            <div style={{ position: "absolute", top: 0, left: '-1em', width: "10em", height: "10em" }}>
                <div className="pickerLabel">{color}</div>
